...
namespace M3D_ISICG
{
	...
	bool LabWork4::init()
	{
		const std::string vertexShaderStr
			= readFile( _shaderFolder + "mesh.vert" ); 
		const std::string fragmentShaderStr
			= readFile( _shaderFolder + "mesh.frag" );			
	...
	void LabWork4::render()
	{
		...
		Mat3f Normal_Matrix = Mat3f( glm::transpose(
			glm::inverse( camera.getViewMatrix() * bunny._transformation ) ) ); 

		

		glProgramUniformMatrix4fv(
			Program, location_MVP, 1, GL_FALSE, glm::value_ptr( MVPa ) ); 
		glProgramUniformMatrix3fv( Program, location_NormalMatrix, 1, GL_FALSE, glm::value_ptr( Normal_Matrix ) );
		// glProgramUniform3fv(Program, glGetUniformLocation( Program, "lightPos" ), 1, glm::value_ptr(
		// camera.Positioncamera()));
		glProgramUniform3fv(
			Program, glGetUniformLocation( Program, "lightPos" ), 1, glm::value_ptr( Vec3f( 0.f, 0.f, 1.f ) ) );
		glProgramUniformMatrix4fv(
