...
namespace M3D_ISICG
{
	...
	bool LabWork3::init()
	{
		...
		// location_ModelMatrix = glGetUniformLocation(Program, "Modelmatrix" ); // Récupérez l’adresse de modelmatrix
		// et le stocker dans location_ModelMatrix location_ViewMatrix = glGetUniformLocation(Program, "Viewmatrix" );
		// // Récupérez l’adresse de ViewMatrix et le stocker dans location_ViewMatrix
		// location_ProjectionMatrix=glGetUniformLocation( Program, "Projectionmatrix" ); // Récupérez l’adresse de
		// ProjectionMatrix et le stocker dans location_ProjectionMatrix
		location_MVP = glGetUniformLocation(
			Program, "MVP" ); // Récupérez l’adresse de modelmatrix et le stocker dans location_MVP
		...
		// );
		// glProgramUniformMatrix4fv(Program, location_ProjectionMatrix, 1, GL_FALSE, glm::value_ptr(
	...
	void LabWork3::animate( const float p_deltaTime )
	{
		...
		glProgramUniformMatrix4fv(
			...
			1,
			GL_FALSE,
			glm::value_ptr( MVPMatrix ) ); // Mettre à jour la variable uniforme MVP dans location_MVP
	}
	...
	LabWork3::Mesh LabWork3::_createCube()
	{
		...
		_cube.couleurs_sommets = {
			getRandomVec3f(), getRandomVec3f(), getRandomVec3f(), getRandomVec3f(),
			getRandomVec3f(), getRandomVec3f(), getRandomVec3f(), getRandomVec3f()
		}; // les couleurs sont aléatoirement sélectionnées.
	...
	void LabWork3::_initBuffers()
	{
		...
		glNamedBufferData( _cube.VBO_c,
						   _cube.couleurs_sommets.size() * sizeof( Vec3f ),
						   _cube.couleurs_sommets.data(),
						   GL_STATIC_DRAW );
		...
		glNamedBufferData( _cube.EBO,
						   _cube.indices_sommets.size() * sizeof( unsigned int ),
						   _cube.indices_sommets.data(),
						   GL_STATIC_DRAW );
		//----------------------------------------------------------------------------------------------------------
		// Liez l’EBO au VAO
	...
	void LabWork3::_initCamera()
	{
		...
		camera.setScreenSize(
			_windowWidth, _windowHeight ); // Initialisez les attributs de la camera relatifs a la taille de la fenetre
